...
- ~~**毎フレームの shadowBlur 付き fillText を排除(P6)**: スプライトキャッシュ(オフスクリーン canvas への焼き込み + drawImage)~~ → **Phase 3 で実装・実測の結果、不採用**。
  - Phase 3 で「テキスト: インスタンス単位スプライト」「H2o: サイズ量子化キーの共有スプライト」を実装したが、実ブラウザ(Chrome/macOS)の A/B 計測で、負荷シナリオ(h=500&o=500、H2o 約1万個滞留)の wall フレーム時間が **82ms(直描き)→ 150〜200ms(スプライト)に退行**した。JS 側の renderFrame 記録時間は 69→57ms と改善しており、退行はコミット後のラスタライズで発生。
  - 切り分けの結果: スプライトの drawImage 単独(テキストのみのシナリオ)は直描きより 1.5〜3.6 倍速いが、**canvas ソースの drawImage と大量のグラデーション fill が同一フレームに共存すると、フレーム全体のラスタライズが遅いパスに落ちる**(描画順のグループ化・スプライト canvas の寸法統一・生成 churn 排除のいずれでも解消せず)。
  - 結論: 描画はテキスト・H2o とも直描き(従来方式)を維持する。ただし Phase 3 の副産物として以下は採用:
    - **kind 別グループ描画**(MizuSimulator): 旧実装の kind 単位の重なり順(H→H2→O→H2o)の再現。
    - **H2o の `shadowBlur = 1` 明示化**(DropletRenderer): 旧実装はテキスト描画が bufferCtx に残す shadowBlur=1 の暗黙持ち越しに依存していた。実効値を明示して持ち越し依存を解消(見た目は不変)。
  - 将来再挑戦する場合は、必ず実ページの rAF タイムスタンプ計測で A/B すること(jsdom や getImageData 強制のマイクロベンチはラスタライズ経路が異なり、逆の結論が出る)。
...
## 付録: Phase 2 計測結果(レビュアーが記入)

計測日 2026-07-05。プロトコルは Phase 0 と同じ(1280×800、リロード後3秒ウォームアップ → rAF タイムスタンプ差分)。**日をまたぐと環境差が大きい**(旧コードの h=500 が 2026-07-04 は 113ms、07-05 再計測では 251ms)ため、比較は同日に worktree で並行起動した旧コード(コミット 1aaeab0)との A/B で行った。

| シナリオ | Phase 2 (ms) | 旧コード同日 (ms) | 備考 |
|---|---|---|---|
| デフォルト | 16.89 | - | 60fps 維持(vsync 律速)。劣化なし |
| h=500, o=500 | 120.27 | 251.38 | **約 2.1 倍高速**。中央値 100.1 / p95 216.8。300 フレーム |
| h=1000, o=1000 | 601.41 | 計測不能 | 旧コードは CDP 全応答不能(フレーム時間が秒オーダー)。Phase 2 は応答可能。300 フレーム |
| h=3000, o=3000 | ~957(参考値) | - | ビーコン方式・初期13フレームの平均。以後 H2o 滞留で 1.2〜2.9 秒/フレームまで増大 |

計測メモ(Phase 2):

- 差し戻し修正1件: 当初実装は全粒子(H2o 含む)を衝突検出に渡しており、H2o が数千個滞留する負荷シナリオで旧コード比 1.6 倍の**退行**だった(h=500 で 400ms)。`ReactionRegistry.reactiveKinds()` によるフィルタ導入後、上表の 120ms まで改善
- 現在の支配項は**滞留 H2o のグラデーション描画**(h=500 で約 9,400 個、h=1000 で約 45,000 個に達する)。衝突判定はもはやボトルネックではない
- §2.5 の 60fps 目標(h=1000)は Phase 2 時点では未達。H2o を含むスプライトキャッシュ(§2.3、Phase 3)適用後に再評価する

## 付録: Phase 3 計測結果(レビュアーが記入)

計測日 2026-07-05。プロトコルは Phase 0/2 と同じ。比較対象の Phase 2 は同日・同環境で worktree 並行起動して再計測した値。

| シナリオ | Phase 3 最終 (ms) | Phase 2 同日 (ms) | 備考 |
|---|---|---|---|
| デフォルト | 16.67 | 16.89 | 60fps 維持(vsync 律速)。renderFrame は 1.8ms → 1.3ms |
| h=500, o=500 | 80.66 | 83.72 | 同等〜わずかに改善。中央値 83.3 / p95 116.7 |
| h=1000, o=1000 | 412.2 | 601.41 | 約 1.5 倍改善(時間帯ノイズ含む可能性あり)。H2o 約 47,000 個滞留時 |
| h=0, o=800(参考) | 35(h=1500,o=0)/ 60fps | 52 / 60fps 際 | テキストのみのシナリオ。スプライト検証時の参考値 |

計測メモ(Phase 3):

- **スプライトキャッシュ(P6)は実装→実測→不採用**(経緯と根拠は §2.3 を参照)。差し戻し4回: (1) H2o の実効 shadowBlur=1 の焼き込み、(2) スプライト canvas 寸法統一、(3) H2o 直描き回帰、(4) kind 別グループ描画 — (1) は忠実性の修正として最終形にも残存、(4) は旧実装の描画順の再現として採用、(2)(3) はスプライト全面撤去により消滅
- 最終形の Phase 3 成果: measureText の static キャッシュ(P5)/ kind 別グループ描画 / H2o の shadowBlur 明示化。負荷シナリオの性能は Phase 2 と同等以上を確認
- §2.5 の 60fps 目標(h=1000)は**未達**。現在の支配項は滞留 H2o(数万個)のグラデーション描画そのもので、Canvas 2D の範囲での改善余地は小さい。60fps 達成には WebGL 化(インスタンス描画)か H2o 滞留数の上限(世界観の変更)が必要であり、いずれも本プランのスコープ外。デフォルトシナリオの 60fps 維持(劣化なし)は達成済み
