...
- **毎フレームの shadowBlur 付き fillText を排除(P6)**: 各粒子は生成時に一度だけ、自分の色・影込みのテキストを小さなオフスクリーン canvas に描き、以後は `drawImage` 1回にする。
  ...
  - **H2o もスプライトキャッシュの対象に含める**(Phase 2 レビュー時に方針変更)。当初「グラデーションが座標依存なので対象外」としていたが、`createRadialGradient` の座標は粒子位置からの相対オフセットのみで決まるため、スプライト化しても描画結果はピクセル同一。かつ Phase 2 の実測で、負荷シナリオでは滞留した H2o(数千〜数万個)のグラデーション描画がフレーム時間の支配項であることが判明した(付録の Phase 2 計測メモ参照)。H2o のサイズは生成時乱数で決まるため、キャッシュはインスタンス単位(`DropletRenderer` が内部に保持)。
